...
<xsl:stylesheet 
    ...
    <xsl:key name="index-sorted" match="//index"
        use="concat(normalize-space(primary),';',normalize-space(secondary),';', normalize-space(tertiary))"/>
            ...
            <fo:flow flow-name="xsl-region-body">
                ...
                <fo:block>
                    ...
                    <xsl:for-each
                        ...
                        <fo:block font-weight="bold" padding-before="3pt"
                        ...
                        <xsl:for-each
                            ...
                            <xsl:if test="$beg = $first">
                                        ...
                                        <xsl:if test="not(see)">
                                            ...
                                            <xsl:call-template name="page-numbers">
                                                <xsl:with-param name="index-key">
                                                  <xsl:value-of
                                                  select="concat(normalize-space(primary),';',normalize-space(secondary),';', normalize-space(tertiary))"
                                                  />
                                                </xsl:with-param>
                                            </xsl:call-template>
                                        </xsl:if>
                                ...
                                <xsl:if test="primary and secondary and not(tertiary)">
                                    <fo:block start-indent="4mm" padding-end="2mm"
                                ...
                                <xsl:if test="primary and secondary and tertiary">
                                    <fo:block start-indent="6mm" padding-end="4mm"
                                        text-align-last="justify">
                                        <xsl:value-of select="normalize-space(tertiary)"/>
                                        <xsl:text>&#xA0;</xsl:text>
                                        <xsl:if test="not(see)">

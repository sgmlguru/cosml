    ...
    <xsl:template match="ftnote[not(ancestor::tgroup)]">
        <fo:footnote>
            <fo:inline font-size="0.63em" baseline-shift="super">
                <xsl:number level="any" count="ftnote[not(ancestor::tgroup)]" format="[1]"
                    from="body|back"/>
            </fo:inline>
            <fo:footnote-body>
                <fo:list-block provisional-distance-between-starts="20pt"
                    ...
                    <xsl:if
                        test="ancestor::note or ancestor::list or ancestor::step-list or ancestor::step-list">
                        <xsl:attribute name="start-indent">0mm</xsl:attribute>
                    </xsl:if>
                    <fo:list-item margin-top="0.1em">
                        <fo:list-item-label end-indent="label-end()">
                            <fo:block font-size="0.83em" line-height="0.9em">
                                <xsl:number level="any" count="ftnote[not(ancestor::tgroup)]"
                                    format="[1]" from="body|back"/>
                            </fo:block>
                        </fo:list-item-label>

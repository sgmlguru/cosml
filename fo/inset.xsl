...
<xsl:stylesheet 
    ...
    <xsl:template name="inset-section1">
        <fo:block space-after="16pt">
            ...
            <fo:block xsl:use-attribute-sets="section-titles" start-indent="-10mm"
                font-size="{$heading1-font-size}" id="{generate-id(.)}">
    ...
    <xsl:template name="inset-section2">
        <fo:block space-before="6pt">
            ...
            <fo:block xsl:use-attribute-sets="section-titles" start-indent="-3mm" font-size="14pt"
                id="{generate-id(.)}">
            ...
            <fo:block>
                ...
                <xsl:if test="not(contains(@xlink:href, '#'))">
                    <xsl:apply-templates
                        select="document(@xlink:href, /)/section/*[not(self::title)]"/>
                </xsl:if>
          ...
          name(document(@xlink:href, /)/*[1]): <xsl:value-of select="name(document(@xlink:href, /)/*[1])"/>
          name(document(substring-before(@xlink:href, '#'), /)/*[1]): <xsl:value-of select="name(document(substring-before(@xlink:href, '#'), /)/*[1])"/>
        </xsl:comment>
    ...
    <xsl:template name="inset-section3">
        <fo:block space-before="6pt">
            ...
            <fo:block xsl:use-attribute-sets="section-titles" start-indent="0mm" font-size="12pt"
                id="{generate-id(.)}">

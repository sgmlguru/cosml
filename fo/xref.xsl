...
<xsl:stylesheet 
            ...
            <xsl:when test="$idref-type='section'">
                ...
                <xsl:if test="$section.numbering=1">
                    <fo:inline font-weight="bold">
                        <xsl:apply-templates select="key('id-nodes', $idref-value)"
                            mode="sect-numbering"/>
                        <xsl:text>, </xsl:text>
                    </fo:inline>
                </xsl:if>
    ...
    <xsl:template match="ftnoteref[not(ancestor::table)]">
        ...
        <xsl:if test="normalize-space(.) != ''">
            <fo:inline font-size="0.63em" baseline-shift="super"
                keep-with-previous.within-line="always" keep-together.within-line="always">
    ...
    <xsl:template match="*" mode="sect-numbering">
        ...
        <xsl:text> </xsl:text>
        <xsl:number level="multiple" format="1.1" count="section" from="body"/>
        <!-- count added för xsltproc -->
    </xsl:template>
    ...
    <xsl:template match="*" mode="app-numbering">
        ...
        <xsl:text> </xsl:text>
        <xsl:number level="multiple" format="A.1" count="section" from="back"/>
        <!-- count added för xsltproc -->
    </xsl:template>

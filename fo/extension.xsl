...
<xsl:stylesheet
  ...
  <xsl:template match="cos" mode="outline">
    <rx:bookmark internal-destination="doc-title">
      ...
      <xsl:apply-templates select="meta-data/abstract|body/section|back/section|//reference|//glossary" mode="outline"/>
  ...
  <xsl:template name="axf-bookmarks">
    ...
    <xsl:attribute name="axf:outline-level">
      <xsl:value-of select="count(ancestor::section) + count(self::back) + count(self::meta-data)"/>
    </xsl:attribute>
  ...
  <xsl:template match="cos" mode="fop.outline">
    <fox:outline internal-destination="{generate-id(.)}">
      ...
      <xsl:apply-templates select="meta-data|body|back"
        mode="fop.outline"/>

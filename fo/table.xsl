...
<xsl:stylesheet 
                            ...
                            <xsl:otherwise>
                                ...
                                <fo:table-column>
                                    <xsl:attribute name="column-width">
                                            proportional-column-width(<xsl:value-of
    ...
    <xsl:template match="tbody/row[1]/entry">
        ...
        <fo:table-cell xsl:use-attribute-sets="tablecell-properties">
            <xsl:call-template name="entry-attributes"/>
            <xsl:if
                test="ancestor::table[1][@frame='none' or @frame='bottom'] and not(ancestor::thead)">
                <xsl:call-template name="table-firstrow"/>
            </xsl:if>
            <xsl:if
                test="ancestor::table[1][@frame='top' or @frame='topbot'] and not(ancestor::thead)">
                <xsl:call-template name="table-firstrow-top"/>
            </xsl:if>
    ...
    <xsl:template
        ...
        <fo:table-cell xsl:use-attribute-sets="tablecell-properties">
            ...
            <xsl:if
                test="ancestor::table[1][@frame='none' or @frame='top'] and not(ancestor::tfoot)">
                <xsl:call-template name="table-lastrow"/>
            </xsl:if>
            <xsl:if
                test="ancestor::table[1][@frame='bottom' or @frame='topbot'] and not(ancestor::tfoot)">
                <xsl:call-template name="table-lastrow-bottom"/>
            </xsl:if>
    ...
    <xsl:template name="entry-attributes">
        ...
        <xsl:if test="@namest">
            <xsl:variable name="endcolnum"
                select="../../../colspec[@colname=current()/@nameend]/@colnum"/>
            <xsl:variable name="startcolnum"
                select="../../../colspec[@colname=current()/@namest]/@colnum"/>
    ...
    <xsl:template name="ftnote-row">
        <fo:table-row>
            <fo:table-cell number-columns-spanned="{@cols}">
                <fo:list-block padding-before="4pt" padding-after="2pt" margin-left="2mm"
                    provisional-distance-between-starts="20pt" provisional-label-separation="5pt">
                    <xsl:for-each select=".//ftnote">
                        <fo:list-item margin-top="0.1em">
                            <fo:list-item-label end-indent="label-end()">
                                <fo:block font-size="0.63em" line-height="0.9em">
                                    <xsl:number level="any" count="ftnote[ancestor::tgroup]"
                                        format="a)" from="tgroup"/>
                                </fo:block>
                            </fo:list-item-label>

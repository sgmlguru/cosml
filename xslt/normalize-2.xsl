...
<xsl:stylesheet 
    ...
    <xsl:template match="section-group">
        ...
        <xsl:if test="not(@applic) or $print='yes'">
            <xsl:choose>
                <xsl:when test="count(ancestor-or-self::*)=1">
                    <section-group>
                        <xsl:apply-templates/>
                    </section-group>
                </xsl:when>
    ...
    <xsl:template name="use-node">
        ...
        <xsl:if test="$i>0">
            <xsl:choose>
                <!-- Applic matches and is the only applic -->
                <xsl:when test="contains(@applic,$compare-string) and
                    (string-length(@applic)=string-length($compare-string))">
                    <xsl:call-template name="use-node">
                    ...
                    <!--<xsl:value-of select="concat(': (lone applic) compare-string: ',$compare-string,' | ')"/>-->
                </xsl:when>
                
                <!-- In the middle -->
                <xsl:when test="matches(@applic,concat('[\s]+',$compare-string,'[\s]+'))">
                    <xsl:call-template name="use-node">
                        <xsl:with-param name="i" select="$i - 1"/>
                        <xsl:with-param name="test-applic">
                            <xsl:value-of
                    ...
                    <xsl:value-of select="'1'"/>
                    <!--<xsl:value-of select="concat(': (in the middle) compare-string: ',$compare-string,' | ')"/>-->
                </xsl:when>
                
                <!-- Starts with, others follow -->
                <xsl:when test="matches(@applic,concat('^',$compare-string,'[\s]+'))">
                    <xsl:call-template name="use-node">
                        <xsl:with-param name="i" select="$i - 1"/>
                        <xsl:with-param name="test-applic">
                            <xsl:value-of
                    ...
                    <xsl:value-of select="'1'"/>
                    <!--<xsl:value-of select="concat(': (starts-with) compare-string: ',$compare-string,' | ')"/>-->
                </xsl:when>
                
                <!-- Ends with, others before -->
                <xsl:when test="matches(@applic,concat('[\s]+',$compare-string,'$'))">
                    <xsl:call-template name="use-node">
                        <xsl:with-param name="i" select="$i - 1"/>
                        <xsl:with-param name="test-applic">
                            <xsl:value-of
                    ...
                    <xsl:value-of select="'1'"/>
                    <!--<xsl:value-of select="concat(': (ends-with) compare-string: ',$compare-string,' | ')"/>-->
                </xsl:when>
                ...
                <xsl:otherwise>
                    ...
                    <!--<xsl:value-of select="concat(': (no match) compare-string: ',$compare-string,' | ')"/>-->
                </xsl:otherwise>
            </xsl:choose>
        </xsl:if>
    </xsl:template>
